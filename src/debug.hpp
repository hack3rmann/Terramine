...
class DebugText {
public:
    DebugText(glm::uvec2 viewport_size);
...
namespace debug {
    ...
    inline auto initialize(glm::uvec2 viewport_size) -> void {
        LINES.emplace();
        TEXT.emplace(viewport_size);
    }
...
struct DebugOwner {
    ...
    inline DebugOwner(glm::uvec2 viewport_size) {
        debug::initialize(viewport_size);
    }
