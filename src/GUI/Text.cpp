...
#define TEXT_VERTEX(I,X,Y,T,S,R,G,B) buffer[I+0] = X; \
									 buffer[I+1] = Y; \
									 buffer[I+2] = T; \
									 buffer[I+3] = S; \
									 buffer[I+4] = R; \
									 ...
									 buffer[I+6] = B; \
									 I += TEXT_VERTEX_SIZE;
#define TEXT_QUAD(I,RX,RY,RW,RH,X,Y,W,H,R,G,B) TEXT_VERTEX(I, RX,      RY + RH, X,     Y,     R, G, B); \
...
Text::Text(std::string text, glm::vec2 position, float fontSize) : text(text) {
	/* init */
	...
	for (unsigned int i = 0; i < text.size(); i++) {
		...
		TEXT_QUAD(vertices,
				  cursorPointer,
				  ((lh - curr.YOffset) - h - lh / 2.0f) / sizeH,
				  rw / sizeW,
				  rh / sizeH,
				  x / sizeW,
				  y / sizeH,
