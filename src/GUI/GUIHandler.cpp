...
GUIHandler::GUIHandler(GUIstate current) {
	...
	GUIs[startMenu]->addButton(0.0f, 0.0f, 1.0f, 0.3f, bDef, bHover, bClicked, "Start", [&]() {
		this->current = nothing;
	});
	GUIs[startMenu]->addButton(0.0f, -0.4f, 1.0f, 0.3f, bDef, bHover, bClicked, "Exit", [&]() {
	...
	GUIs[pauseMenu]->addButton(0.0f, 0.2f, 1.0f, 0.3f, bDef, bHover, bClicked, "Return", [&]() {
		this->current = nothing;
		Events::toggleCursor();
	});
	GUIs[pauseMenu]->addButton(0.0f, -0.2f, 1.0f, 0.3f, bDef, bHover, bClicked, "Exit", [&]() {
		this->current = startMenu;
	});
}
